/*
 * Infusionsoft REST API
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: V1
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */


package com.infusionsoft.model;

import java.util.Objects;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import com.infusionsoft.model.InfusionsoftHistoricalCounts;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

/**
 * InfusionsoftGoal
 */
@javax.annotation.Generated(value = "io.swagger.codegen.languages.JavaClientCodegen", date = "2018-10-16T20:44:31.010Z")
public class InfusionsoftGoal {
  @SerializedName("historical_contact_counts")
  private InfusionsoftHistoricalCounts historicalContactCounts = null;

  @SerializedName("id")
  private Long id = null;

  @SerializedName("name")
  private String name = null;

  @SerializedName("next_sequence_ids")
  private List<Long> nextSequenceIds = null;

  @SerializedName("previous_sequence_ids")
  private List<Long> previousSequenceIds = null;

  /**
   * Gets or Sets type
   */
  @JsonAdapter(TypeEnum.Adapter.class)
  public enum TypeEnum {
    WEBFORM("WebForm"),
    
    LANDINGPAGE("LandingPage"),
    
    SURVEY("Survey"),
    
    LINKCLICK("LinkClick"),
    
    SCORE("Score"),
    
    PURCHASE("Purchase"),
    
    QUOTE("Quote"),
    
    WEBSITETRIGGER("WebsiteTrigger"),
    
    INTERNALFORM("InternalForm"),
    
    TASK("Task"),
    
    STAGEMOVE("StageMove"),
    
    NOTE("Note"),
    
    TAG("Tag"),
    
    GROSOCIAL("GroSocial"),
    
    TWITTERFORM("TwitterForm"),
    
    API("API"),
    
    WORDPRESS("WordPress"),
    
    NEWLANDINGPAGE("NewLandingPage");

    private String value;

    TypeEnum(String value) {
      this.value = value;
    }

    public String getValue() {
      return value;
    }

    @Override
    public String toString() {
      return String.valueOf(value);
    }

    public static TypeEnum fromValue(String text) {
      for (TypeEnum b : TypeEnum.values()) {
        if (String.valueOf(b.value).equals(text)) {
          return b;
        }
      }
      return null;
    }

    public static class Adapter extends TypeAdapter<TypeEnum> {
      @Override
      public void write(final JsonWriter jsonWriter, final TypeEnum enumeration) throws IOException {
        jsonWriter.value(enumeration.getValue());
      }

      @Override
      public TypeEnum read(final JsonReader jsonReader) throws IOException {
        String value = jsonReader.nextString();
        return TypeEnum.fromValue(String.valueOf(value));
      }
    }
  }

  @SerializedName("type")
  private TypeEnum type = null;

  public InfusionsoftGoal historicalContactCounts(InfusionsoftHistoricalCounts historicalContactCounts) {
    this.historicalContactCounts = historicalContactCounts;
    return this;
  }

   /**
   * Get historicalContactCounts
   * @return historicalContactCounts
  **/
  @ApiModelProperty(value = "")
  public InfusionsoftHistoricalCounts getHistoricalContactCounts() {
    return historicalContactCounts;
  }

  public void setHistoricalContactCounts(InfusionsoftHistoricalCounts historicalContactCounts) {
    this.historicalContactCounts = historicalContactCounts;
  }

  public InfusionsoftGoal id(Long id) {
    this.id = id;
    return this;
  }

   /**
   * Get id
   * @return id
  **/
  @ApiModelProperty(value = "")
  public Long getId() {
    return id;
  }

  public void setId(Long id) {
    this.id = id;
  }

  public InfusionsoftGoal name(String name) {
    this.name = name;
    return this;
  }

   /**
   * Get name
   * @return name
  **/
  @ApiModelProperty(value = "")
  public String getName() {
    return name;
  }

  public void setName(String name) {
    this.name = name;
  }

  public InfusionsoftGoal nextSequenceIds(List<Long> nextSequenceIds) {
    this.nextSequenceIds = nextSequenceIds;
    return this;
  }

  public InfusionsoftGoal addNextSequenceIdsItem(Long nextSequenceIdsItem) {
    if (this.nextSequenceIds == null) {
      this.nextSequenceIds = new ArrayList<Long>();
    }
    this.nextSequenceIds.add(nextSequenceIdsItem);
    return this;
  }

   /**
   * Get nextSequenceIds
   * @return nextSequenceIds
  **/
  @ApiModelProperty(value = "")
  public List<Long> getNextSequenceIds() {
    return nextSequenceIds;
  }

  public void setNextSequenceIds(List<Long> nextSequenceIds) {
    this.nextSequenceIds = nextSequenceIds;
  }

  public InfusionsoftGoal previousSequenceIds(List<Long> previousSequenceIds) {
    this.previousSequenceIds = previousSequenceIds;
    return this;
  }

  public InfusionsoftGoal addPreviousSequenceIdsItem(Long previousSequenceIdsItem) {
    if (this.previousSequenceIds == null) {
      this.previousSequenceIds = new ArrayList<Long>();
    }
    this.previousSequenceIds.add(previousSequenceIdsItem);
    return this;
  }

   /**
   * Get previousSequenceIds
   * @return previousSequenceIds
  **/
  @ApiModelProperty(value = "")
  public List<Long> getPreviousSequenceIds() {
    return previousSequenceIds;
  }

  public void setPreviousSequenceIds(List<Long> previousSequenceIds) {
    this.previousSequenceIds = previousSequenceIds;
  }

  public InfusionsoftGoal type(TypeEnum type) {
    this.type = type;
    return this;
  }

   /**
   * Get type
   * @return type
  **/
  @ApiModelProperty(value = "")
  public TypeEnum getType() {
    return type;
  }

  public void setType(TypeEnum type) {
    this.type = type;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    InfusionsoftGoal goal = (InfusionsoftGoal) o;
    return Objects.equals(this.historicalContactCounts, goal.historicalContactCounts) &&
        Objects.equals(this.id, goal.id) &&
        Objects.equals(this.name, goal.name) &&
        Objects.equals(this.nextSequenceIds, goal.nextSequenceIds) &&
        Objects.equals(this.previousSequenceIds, goal.previousSequenceIds) &&
        Objects.equals(this.type, goal.type);
  }

  @Override
  public int hashCode() {
    return Objects.hash(historicalContactCounts, id, name, nextSequenceIds, previousSequenceIds, type);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class InfusionsoftGoal {\n");
    
    sb.append("    historicalContactCounts: ").append(toIndentedString(historicalContactCounts)).append("\n");
    sb.append("    id: ").append(toIndentedString(id)).append("\n");
    sb.append("    name: ").append(toIndentedString(name)).append("\n");
    sb.append("    nextSequenceIds: ").append(toIndentedString(nextSequenceIds)).append("\n");
    sb.append("    previousSequenceIds: ").append(toIndentedString(previousSequenceIds)).append("\n");
    sb.append("    type: ").append(toIndentedString(type)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

